Hi there 

<!-- Terminal Görünümlü Hakkımda Kısmı -->

> whoami
Bilgisayar Mühendisliği Öğrencisi

> cat skills.txt
• Mobile  : Flutter (Dart), Kotlin
• Backend : Node.js
• Data    : PostgreSQL, MongoDB, SQL
• DevOps  : Docker, Linux, CI/CD

> echo "Vizyonum"
"Teknik yetkinliği estetik bir vizyonla birleştirerek,
uçtan uca kullanıcı deneyimi sunan uygulamalar inşa etmek."


🛠️ Kullandığım Teknolojiler

<p align="left">
<img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white" />
<img src="https://www.google.com/search?q=https://img.shields.io/badge/Dart-0175C2%3Fstyle%3Dfor-the-badge%26logo%3Ddart%26logoColor%3Dwhite" />
<img src="https://www.google.com/search?q=https://img.shields.io/badge/Node.js-43853D%3Fstyle%3Dfor-the-badge%26logo%3Dnode.js%26logoColor%3Dwhite" />
<img src="https://www.google.com/search?q=https://img.shields.io/badge/PostgreSQL-316192%3Fstyle%3Dfor-the-badge%26logo%3Dpostgresql%26logoColor%3Dwhite" />
<img src="https://www.google.com/search?q=https://img.shields.io/badge/Docker-2496ED%3Fstyle%3Dfor-the-badge%26logo%3Ddocker%26logoColor%3Dwhite" />
<img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
</p>

🚀 Projelerim

1. DermaPharma IMS

Flutter, Node.js, PostgreSQL




Doktor, hasta ve eczacıları tek bir dijital ekosistemde buluşturan; reçete takibi ve randevu yönetimini sağlayan entegre sağlık platformu.

2. Planner App

Node.js, MongoDB, Docker




Kullanıcıların zaman yönetimini optimize eden, Docker ile konteynerize edilmiş, ölçeklenebilir ve modern bir planlama asistanı.

3. DevOps & Cloud Infrastructure

Azure, Kubernetes, CI/CD




Modern yazılım dağıtım süreçlerini simüle eden, otomatikleştirilmiş bulut altyapı projesi.

<!-- Ziyaretçi Sayacı (Pembe Tema) -->

<p align="center">
<img src="https://komarev.com/ghpvc/?username=[github_kullanici_adin]&style=flat-square&color=ff69b4&label=PROFILE+VIEWS" alt="Profil Ziyaretçileri" />
</p>
